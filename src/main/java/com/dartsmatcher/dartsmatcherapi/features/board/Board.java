...
public class Board {
	...
	private final ArrayList<BoardSectionAreaDimen> areaDimensions = new ArrayList<>(Arrays.asList(
			new BoardSectionAreaDimen(BoardSectionArea.DOUBLE_BULL, 0, 6),
			new BoardSectionAreaDimen(BoardSectionArea.SINGLE_BULL, 6, 16),
			new BoardSectionAreaDimen(BoardSectionArea.INNER_SINGLE, 16, 99),
			new BoardSectionAreaDimen(BoardSectionArea.TRIPLE, 99, 107),
			new BoardSectionAreaDimen(BoardSectionArea.OUTER_SINGLE, 107, 162),
			new BoardSectionAreaDimen(BoardSectionArea.DOUBLE, 162, 170),
			new BoardSectionAreaDimen(BoardSectionArea.MISS, 170, Integer.MAX_VALUE)
	));

	/**
	 * @param cartesianCoordinate CartesianCoordinate The cartesian coordinates of which a score needs to be calculated.
	 * @return int The score corresponding the the polar coordinates.
	 */
	public int getScoreCartesian(CartesianCoordinate cartesianCoordinate) {
	 ...
	 * @return int The score corresponding the the polar coordinates.
	 */
	public int getScorePolar(PolarCoordinate polarCoordinate) {
		int section = getSection(polarCoordinate.getThetaNormalized());
		BoardSectionArea sectionArea = getSectionArea(polarCoordinate.getR());
		...
		return score * sectionArea.getMultiplier();
	}
	...
	/**
	 * @param sectorIndex int the index of the sector counting counter clockwise starting at 0 = upper half of 6, 1 = 13 etc.
	 ...
	 */
	public BoardSectionArea getSectionArea(double r) {

		for (BoardSectionAreaDimen areaDimension : areaDimensions) {
			if (r >= areaDimension.getInner() && r < areaDimension.getOuter()) return areaDimension.getSectionArea();
		}

		return BoardSectionArea.MISS;
	}
	 ...
	 */
	public PolarCoordinate getCenter(int section, BoardSectionArea sectionArea) {
		if (sectionArea.equals(BoardSectionArea.DOUBLE_BULL)) return new PolarCoordinate(0, 0);

		return getAreaDimensions().stream()
				.filter(boardSectionAreaDimen -> boardSectionAreaDimen.getSectionArea().equals(sectionArea))
				.findFirst()
				.map(boardSectionAreaDimen -> {
					...
					double r = (boardSectionAreaDimen.getInner() + (boardSectionAreaDimen.getOuter() - 1)) / 2.0;
