...
public class WebSocketEventPublisherImpl implements IWebSocketEventPublisher {
     ...
     *
     * @param destination the WebSocket destination to broadcast to
     * @param messageType the type of the WebSocket message
     * @param payload the payload of the WebSocket message
     * @param <E> the enum type used to identify the WebSocket event
     * @param <P> the payload type
     */
    @Override
    public <E extends Enum<E>, P> void broadcast(
            String destination,
            E messageType,
            P payload
    ) {
        eventPublisher.publishEvent(
                new WebSocketBroadcastEvent<>(
                        destination,
                        messageType,
                        payload
                )
     ...
     */
    @Override
    public <E extends Enum<E>, P> void sendToUser(
            E messageType,
            P payload,
            String sessionId,
            String publishId
